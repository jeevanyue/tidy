#'Automatically detects numeric variables and gives a comprehensive summary, https://github.com/ujjwalkarn/xda
...
numSummary <- function(df){
  ...
  for (var in 1:ncol(df)) {
    ...
    }else if (class(df[[var]])=="Date" || class(df[[var]])=="POSIXt" || class(df[[var]])=="POSIXct") {
      date  <- c(char,names(df[var]))
    }
  }
